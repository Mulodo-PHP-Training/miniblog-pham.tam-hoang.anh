...
class Auth_Login_Simpleauth extends \Auth_Login_Driver
{
	...
	public function create_user($username, $password, $email, $group = 1, Array $profile_fields = array())
	{
		...
		$user = array(
			...
			'email'				=> $email,
			'group'				=> (int) $group,
			'firstname'			=> (string) $profile_fields['firstname'],
			'lastname'			=> (string) $profile_fields['lastname'],
			'avatar'			=> (string) $profile_fields['avatar'],
			'birthday'			=> (string) $profile_fields['birthday'],
			'gender'			=> (int) $profile_fields['gender'],
			'address'			=> (string) $profile_fields['address'],
			'city'				=> (string) $profile_fields['city'],
			'mobile'			=> (string) $profile_fields['mobile'],
			'last_login'		=> 0,
			'login_hash'		=> '',
			'created_at'		=> (string) date('Y-m-d H:i:s',time()),
			'updated_at'		=> (string) date('Y-m-d H:i:s',time())
		);
